...
elseif method == 6
    n_test_ex_analog = zeros(length(m), length(n),4);
end
...
%                 location(true_loc) =1;
                if method == 1    
                    [beam_loc, n_steps, ~] = Hwang_gt(n(i), m(j),1:n(i),{}, location, 0, pmd(k), pfa(k), 1);
                ...
                elseif method ==6
                    [beam_loc, n_steps] = exhaustive(n(i), m(j), location, pmd(k), pfa(k));
                    n_test_ex_analog(j,i,k) = n_test_ex_analog(j,i,k) +n_steps;
                ...
                if isempty(res_beam)
                     ...
                     if sum(true_loc == res_beam', 'all') == 0
                         blockage_Hwang(j,i,k) = blockage_Hwang(j,i,k) +1;
                         md_hwang(j,i,k) = md_hwang(j,i,k) + m(j);                     
                    elseif sum(true_loc == res_beam', 'all') ~= length(true_loc)
                         md_hwang(j,i,k) = md_hwang(j,i,k) + abs(length(true_loc) - sum(true_loc == res_beam', 'all'));
...
elseif method ==5
  ...
  elseif method ==6
    n_test_ex_analog = n_test_ex_analog./trial;
    save("final_n_test_ex_analog_5m" + m+ ".mat", 'n_test_ex_analog')    
    save("final_blockage_ex_analog_5m" + m + ".mat", 'blockage_Hwang' )
    save("final_md_ex_analog_5m" + m+ ".mat", 'md_hwang' )  
end
